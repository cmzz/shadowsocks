...
class DbTransfer(object):
    ...
    def pull_db_all_user():
        ...
        conn = cymysql.connect(host=Config.MYSQL_HOST, port=Config.MYSQL_PORT, user=Config.MYSQL_USER,
                               passwd=Config.MYSQL_PASS, db=Config.MYSQL_DB, charset='utf8')
        cur = conn.cursor()
        cur.execute("SELECT port, u, d, transfer_enable, passwd, switch, enable, vip_type FROM user")
        rows = []
    ...
    def del_server_out_of_bound_safe(rows):
        ...
        for row in rows:
            ...
            if ServerPool.get_instance().server_is_run(row[0]) > 0:
                ...
                if row[6] != -9 and (int(row[7]) != Config.SERVER_TYPE or row[6] != 1):
                ...
                if row[6] == -9 or (int(row[7]) == Config.SERVER_TYPE and row[6] == 1):
